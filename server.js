...
app.get('/api/health', (req, res) => {
  res.json({ 
    ...
    model: process.env.OPENROUTER_MODEL || 'anthropic/claude-3.5-sonnet (default)'
  });
});
...
app.get('/api/models', async (req, res) => {
  try {
    ...
    if (!response.ok) {
      return res.status(500).json({ error: 'Не удалось получить список моделей' });
    }
...
app.listen(PORT, () => {
  ...
  console.log(`🤖 Модель: ${process.env.OPENROUTER_MODEL || 'anthropic/claude-3.5-sonnet (default)'}`);
});
